...
export default function VerificationDetailPage({ params }) {
  ...
  const fetchVerificationDetails = async (id, userId) => {
    try {
      ...
      const response = await fetch(`https://unlimiteddatagh.onrender.com/api/verifications/${id}?userId=${userId}`);
  ...
  const fetchVerificationCode = async (id, userId) => {
    try {
      const response = await fetch(`https://unlimiteddatagh.onrender.com/api/verifications/${id}/code?userId=${userId}`);
